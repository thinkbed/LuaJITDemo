...
void doFFITdrTest(lua_State* L)
{
    ...
    {
        printf("Can't get callTDRTest function\n");
    }

    printf("f=%f\n", c.m_b.f);
}

void ffiTest(const char* script_file)
{
    Person p("Tom", 18);
    ...
    /* Load the file containing the script we are going to run */
    int status = luaL_dofile(L, script_file);
    if (status) {
        /* If something went wrong, error message is at the top of */
        /* the stack */
        fprintf(stderr, "Couldn't load file: %s\n", lua_tostring(L, -1));
    ...
    if(strcmp(script_file, "ffi_tdr.lua") == 0)
    {
        doFFITdrTest(L);
    }
}
